...
import { LayoutGenerationError, generateLayouts } from "./generateLayouts.ts";
  ...
  assert.throws(
    () =>
      generateLayouts({
        ...baseRequest,
        intent: { ...baseRequest.intent, mode: "ai" },
        options: { ...baseRequest.options, allowFallback: false },
      }),
    LayoutGenerationError,
  );
});

test("template mode falls back when the requested style has no supported templates", () => {
  const response = generateLayouts({
    ...baseRequest,
    canvas: { width: 1920, height: 1080, ratioId: "16:9" },
    intent: {
      ...baseRequest.intent,
      mode: "template",
      style: "portrait-triptych",
    },
    options: { ...baseRequest.options, allowFallback: true },
  });

  assert.equal(response.source, "fallback");
  assert.ok(response.warnings?.[0].includes("Template mode produced no valid"));
  assert.ok(response.candidates.length > 0);
});

test("template mode throws when no supported template exists and fallback is disabled", () => {
  assert.throws(
    () =>
      generateLayouts({
        ...baseRequest,
